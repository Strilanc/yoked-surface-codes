...
import functools
import pathlib
from typing import Iterable, Set, Dict, Callable, Union, FrozenSet, Tuple, Literal, TYPE_CHECKING, Optional
...
class PatchOutline:
        ...
        for i in range(len(self.observables)):
            for j in range(len(self.observables)):
                ...
                if does_commute != should_commute:
                    raise ValueError(f"Bad commutation relationship between X{i} and Z{j}: {pts_x=} vs {pts_z=}")
        ...
        for obs_pair in self.observables:
            for basis, obs in obs_pair:
                ...
                if obs.end_point_set - same_boundary_points:
                    raise ValueError(f"Observable doesn't terminate on same-basis boundaries:  {basis=} {obs=}.")
                if not (obs.int_point_set - same_boundary_points):
                    raise ValueError(f"Observable is a stabilizer (only runs along same-basis boundaries): {basis=} {obs=}.")
    ...
    def interior_set(self, *, include_boundary: bool) -> Set[complex]:
        ...
        return int_points_inside_polygon_set([
            c.points for c in self.region_curves
    ...
    def write_svg(
            ...
            *,
            ...
            other: Union['PatchOutline', 'PatchTransitionOutline', Iterable[Union['PatchOutline', 'PatchTransitionOutline']]] = (),
    ) -> None:
    ...
    def to_patch(
            self,
            *,
            rel_order_func: Optional[Callable[[complex], Iterable[complex]]] = None,
    ) -> Patch:
